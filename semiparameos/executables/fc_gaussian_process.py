...
def get_consistent_metamodel(stitch_density, path="./eos_mm+chi+PSR/eos0.out"):
    """
    ...
    metamodel = pd.read_csv(path, names=["number_density", "energy_densityc2","pressure_nuclear"], sep = r"\s+", skiprows = 2)
    ### Change energy density column to CGS units ################################
    metamodel["energy_densityc2"] = metamodel["energy_densityc2"].apply(lambda x: x * MeV_to_gcm3)
    # print(metamodel["energy_densityc2"])
    ##############################################################################
    ...
    #### DEBUGGING
    # print("Converted metamodel energy_density to CGS: ",metamodel_cgs["energy_densityc2"])
    # print("Converted metamodel baryon_density to CGS: ",metamodel_cgs["baryon_density"])
    
    rho_low_interp = np.geomspace(2e8, stitch_density, 400)
...
def get_example_draws(gp, metamodel, samples=10000):
...
def get_full_and_restricted_mm_extensions(metamodel_number, gp, crust_eos_dir,
                                          crust_eos_file,
                                          rho_trans,
                                          random_mm = False,
                                          creation_index = None,
    ...
    * *this will break the conditioning process* * 
    ...
    crust_consistent = get_consistent_metamodel(stitch_density = rho_trans, path = f"{crust_eos_dir}/{crust_eos_file}")
    ...
    eoss_mod = get_example_draws(gp, crust_consistent)
    
    if random_mm:
        metamodel_number = creation_index
    
    mod_outdir = os.path.join(overall_outdir, f"FixedCrustGP-0")
...
if __name__ == "__main__":
    ...
    GP_onset = rho_nuc_in_cgs 
    ### Range of density regime to create the GP in: 
    rho_vals_to_interp = np.geomspace(GP_onset, 10*rho_nuc_in_cgs, 300)
    ...
    gp = fc_gaussian_process.GP.from_samples(eos_samples, np.log(rho_vals_to_interp))
    #np.set_printoptions(threshold = np.inf)
    #print(gp.mean)
    ### Metamodel EoS directory
    crust_eos_dir = "/home/sunny.ng/fixed_crust/nuclear_eos"
    ### Need to create an outdir to save EoS's into ####################################
    result_folder = "Sly4_RG_test_GP"
    overall_outdir = f"/home/sunny.ng/fixed_crust/result/{result_folder}"
    ####################################################################################
    ### Hyperparameters ################################################################
    ...
    if verbose:
        print(f"Generating extensions for crust EoS...")

    ### Main executable for generating EoS set
    get_full_and_restricted_mm_extensions(gp = gp,
                                          rho_trans = rho_vals_to_interp[0],
                                          crust_eos_file = "SLY4_RG.out",
                                          crust_eos_dir = crust_eos_dir,
                                          metamodel_number = None,
                                          rational_quad = True,
                                          gamma = gp_hyp_gamma,
                                          alpha = gp_hyp_alpha,
